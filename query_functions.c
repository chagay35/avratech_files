...
extern void update(struct Client *node,int num, char first[],char last[],char phone[],double debt,unsigned int tm[]);
...
void set(struct Client *head,char line[])
{
	...
	for (i=0,k = 0; k < 6; k++) 
	{
		...
		for(j=0;isalnum(line[i])|| line[i] =='.' || line[i] =='+'|| line[i] =='-' || line[i] =='/';i++,j++)
